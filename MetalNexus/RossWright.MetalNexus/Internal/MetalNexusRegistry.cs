    ...
    : IMetalNexusRegistry
{
    ...
    public void AddEndpoints(params Type[] types)
    {
        if (_isSealed)
            throw new InvalidOperationException(
                "Cannot call AddEndpoints after the MetalNexus middleware has been built. " +
                "Call AddMetalNexusEndpoints during service registration (before UseMetalNexusServer).");
    ...
    public IEndpoint? DefineEndpoint(Type type)
    {
        ...
        endpoint.AuthorizationPolicy = authAttribute?.Policy;
        ...
        endpoint.ProducedErrorTypes = requestType
                .GetCustomAttributes<ProducesErrorAttribute>(inherit: false)
                .Select(_ => _.ExceptionType)
                .ToArray();

        endpoint.SuccessStatusCode = apiRequestAttribute.SuccessStatusCode;

        _bootstrapLogger?.LogTrace($"{requestType.FullName} {endpoint.HttpMethod} {endpoint.Path} " +
    ...
    internal (string, bool) DeterminePath(Type requestType, ApiRequestAttribute apiRequestAttribute)
    {
        ...
        if (path == null)
        {
            ...
            path = path != null
                ? $"{_endpointSchema.ApiPathPrefix}/{path}/{requestName}"
                : $"{_endpointSchema.ApiPathPrefix}/{requestName}";
    ...
    internal string DetermineTag(Type requestType, ApiRequestAttribute attribute, string path)
    {
        ...
        if (_customEndpointSchema != null)
        {
            try
            {
                tag = _customEndpointSchema.DetermineTag(requestType, tag);
            }
            catch (Exception ex)
            {
                throw new MetalNexusException($"Failed to determine tag for {requestType}", ex);
            }
        }
        return tag;
    }
    ...
    internal class Endpoint : IEndpoint
    {
        ...
        public Type[] ProducedErrorTypes { get; set; } = [];
        public System.Net.HttpStatusCode? SuccessStatusCode { get; set; }
    }
}
