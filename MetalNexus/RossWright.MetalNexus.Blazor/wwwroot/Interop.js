window.RossWrightFileInput = {
    ...
    uploadFiles: function (dotNetObj, httpMethod, url, accessToken, fileEntries) {
        var formData = new FormData();
        // Build a parallel array of {name, size} for per-file progress partitioning.
        var fileList = [];
        for (let entry of fileEntries) {
            var file = window.RossWrightFileInput.files[entry.fileRefId];
            formData.append(entry.fieldName, file.data);
            fileList.push({ name: file.data.name, size: file.data.size });
        }
        var ajaxOptions = {
            url: url,
            method: httpMethod,
            ...
            xhr: function () {
                ...
                xhr.upload.addEventListener('progress', function (e) {
                    if (e.lengthComputable) {
                        // Partition aggregate loaded bytes across files in submission order.
                        var remaining = e.loaded;
                        var perFile = fileList.map(function (f) {
                            var fileLoded = Math.min(remaining, f.size);
                            remaining -= fileLoded;
                            return { FileName: f.name, Loaded: fileLoded, Total: f.size };
                        });
                        dotNetObj.invokeMethod('OnProgress', e.loaded, e.total, perFile);
                    }
                }, false);
                return xhr;
            }
