    ...
    : IMetalGuardianTotpMfaService
{
    public async Task<string> GetSetupQrCode(Guid userId, CancellationToken cancellationToken)
    {
        ...
        // Option C: if TOTP is already enabled, caller must reset the user first.
        // Showing the setup QR for an active enrollment without an explicit reset would
        // expose the live secret without an admin-level action.
        if (((ITotpMfaAuthenticationUser)dbUser).IsMfaTotpEnabled)
            throw new MetalGuardianException(
                "TOTP MFA is already enabled for this user. Call ResetUser before re-enrolling.");
    ...
    public async Task<AuthenticationTokens?> VerifyCode(Guid userId, string code, string? deviceFingerprint, CancellationToken cancellationToken)
    {
        ...
        var dbUser = await _authRepo.UpdateUser(userId, dbUser =>
        {
            ...
            if (!mfaUser.IsMfaTotpEnabled && isVerified)
            {
