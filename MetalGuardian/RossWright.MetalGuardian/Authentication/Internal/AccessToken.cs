...
internal class AccessToken : IAuthenticationInformation
{
    ...
    /// <summary>
    /// Returns a parsed <see cref="AccessToken"/> or <c>null</c> if the token string is
    /// null, empty, or malformed. Use at call sites where an invalid token should be
    /// treated as "not authenticated" rather than an exceptional error.
    /// </summary>
    internal static AccessToken? TryCreate(string? accessToken)
    {
        if (string.IsNullOrWhiteSpace(accessToken)) return null;
        try { return new AccessToken(accessToken); }
        catch { return null; }
    }
