...
internal class OtpService : IOtpService
{
    ...
    public async Task SendOtpViaEmail(string userIdentifier, Func<string, IAddressedEmail> makeEmail, CancellationToken cancellationToken = default)
    {
        ...
        {
            await _otpRepo.RemoveOtp(userIdentifier, otp, cancellationToken);
            throw;
        }
    }
    ...
    public async Task<OtpVerifyResult> VerifyOtp(string userIdentifier, string otp,
        bool preserveOtp = false,
        CancellationToken cancellationToken = default)
    {
        var result = await _otpRepo.VerifyOtp(userIdentifier, otp, cancellationToken);
        if (result == OtpVerifyResult.Valid && !preserveOtp)
            await _otpRepo.RemoveOtp(userIdentifier, otp, cancellationToken);
        return result;
    }
    ...
    public Task RemoveOtp(string userIdentifier, string otp,
        CancellationToken cancellationToken = default) =>
        _otpRepo.RemoveOtp(userIdentifier, otp, cancellationToken);
}
