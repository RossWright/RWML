    ...
    : IMetalGuardianAuthenticationService
{
    public async Task<AuthenticationTokens> Login(string userIdentity, string password, string? deviceFingerprint, CancellationToken cancellationToken)
    {
        ...
        // isKnownDevice outcomes:
        //   null  — device repo not registered, OR repo registered but no fingerprint supplied
        //           (non-browser clients, or Blazor before JS loads). MFA providers receive null.
        //   true  — fingerprint matched a known, non-expired device record
        //   false — fingerprint not found or device record expired
    ...
    public async Task<AuthenticationTokens> Refresh(AuthenticationTokens tokens, CancellationToken cancellationToken = default)
    {
        ...
        // notAuthenticatedThrown guards the case where the UpdateRefreshToken
        // implementation catches the callback exception and returns null rather than
        // re-throwing. user == null alone is ambiguous — it is also the result when
        // the refresh token is simply not found.
