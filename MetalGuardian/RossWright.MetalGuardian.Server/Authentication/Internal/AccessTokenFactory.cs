...
internal class AccessTokenFactory : IAccessTokenFactory
{
    public AccessTokenFactory(IMetalGuardianServerConfiguration configuration)
    {
        ...
        // ValidateLifetime = false is intentional: the Refresh flow validates the signature
        // of an expired token to identify the user. Do not change this to true.
    ...
    /// <summary>
    /// Validates the signature, issuer, and audience of an access token without checking
    /// expiration. Intentional — the Refresh flow requires accepting expired-but-validly-signed
    /// tokens. Not for use in request authorization; use <see cref="StrictTokenValidationParameters"/>
    /// for that purpose.
    /// </summary>
